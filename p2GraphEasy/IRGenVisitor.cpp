...
void IRGenVisitor::visitGraphComprehension(GraphComprehensionNode *GC)
{
    ...
    llvm::Value *customNodesBitmap = nullptr;
    ...
    if (GC->condition && GC->condition->op == GraphConditionOp::VertexInSet)
    {
        auto it = NamedValues.find(GC->condition->setName);
        if (it == NamedValues.end())
            throw std::runtime_error("Undefined set variable in vertex-in graph comprehension: " + GC->condition->setName);

        customNodesBitmap = Builder.CreateLoad(bitmapPtrTy, it->second, GC->condition->setName + ".load");
    }
    else if (GC->condition && GC->condition->op == GraphConditionOp::EdgeHas)
    {
        llvm::FunctionType *edgeHasFT = llvm::FunctionType::get(
            bitmapPtrTy,
            {i64Ty,
             llvm::PointerType::getUnqual(i64Ty),
             llvm::PointerType::getUnqual(i32Ty),
             i32Ty},
            false);
        llvm::FunctionCallee edgeHasDecl = Module.getOrInsertFunction("graph_edge_has_nodes_runtime", edgeHasFT);
        ...
        llvm::Value *vertexVal = visitExpr(GC->condition->expr.get());
        if (vertexVal->getType() == i64Ty)
            vertexVal = Builder.CreateTrunc(vertexVal, i32Ty, "edge_has_vertex_i32");
        else if (vertexVal->getType() != i32Ty)
            vertexVal = Builder.CreateIntCast(vertexVal, i32Ty, true, "edge_has_vertex_i32");
    ...
    else if (GC->condition)
    {
        ...
        std::vector<llvm::Value *> tokenArg1;
        std::vector<llvm::Value *> tokenArg2;
        ...
        std::function<void(const std::shared_ptr<GraphConditionNode> &)> emitPostfix =
            [&](const std::shared_ptr<GraphConditionNode> &cond) {
                ...
                case GraphConditionOp::Connected:
                {
                    ...
                    tokenArg1.push_back(llvm::ConstantInt::get(i32Ty, mappedId));
                    tokenArg2.push_back(llvm::ConstantInt::get(i32Ty, 0));
                    break;
                }
                ...
                case GraphConditionOp::Degree:
                {
                    ...
                    tokenArg1.push_back(llvm::ConstantInt::get(i32Ty, opCode));
                    tokenArg2.push_back(llvm::ConstantInt::get(i32Ty, cond->degreeValue));
                    break;
                }
                case GraphConditionOp::EdgeHas:
                {
                    llvm::Value *vertexVal = visitExpr(cond->expr.get());
                    if (vertexVal->getType() == i64Ty)
                        vertexVal = Builder.CreateTrunc(vertexVal, i32Ty, "edge_has_vertex_i32");
                    else if (vertexVal->getType() != i32Ty)
                        vertexVal = Builder.CreateIntCast(vertexVal, i32Ty, true, "edge_has_vertex_i32");
                    tokenKinds.push_back(6);
                    tokenArg1.push_back(vertexVal);
                    tokenArg2.push_back(llvm::ConstantInt::get(i32Ty, 0));
                    break;
                }
        ...
        for (size_t i = 0; i < tokenKinds.size(); ++i)
        {
            ...
            Builder.CreateStore(tokenArg1[i], arg1Ptr);
            Builder.CreateStore(tokenArg2[i], arg2Ptr);
        }
    ...
    GraphNodesMap[GC->targetName] = customNodesBitmap ? customNodesBitmap
                                                      : Builder.CreateCall(getNodeBmFn, {srcGraphPtr}, GC->targetName + "_nodes_bm");
...
void IRGenVisitor::visitAssignment(AssignmentStmtNode *assign)
{
    ...
    if (assign->lhs->type == ASTNodeType::Variable)
    {
        ...
        if (allocTy == getBitmapPtrTy(Context))
        {
            llvm::FunctionType *cloneFT = llvm::FunctionType::get(allocTy, {allocTy}, false);
            auto cloneFn = Module.getOrInsertFunction("roaring_bitmap_clone_runtime", cloneFT);
            if (rhsVal->getType() != allocTy)
                rhsVal = Builder.CreateBitCast(rhsVal, allocTy);
            rhsVal = Builder.CreateCall(cloneFn, {rhsVal}, "set.assign.clone");
        }
...
llvm::Value *IRGenVisitor::visitVarDecl(VarDeclNode *decl)
{
    ...
    auto cloneSetValue = [&](llvm::Value *bitmap) -> llvm::Value * {
        auto *bitmapPtrTy = getBitmapPtrTy(Context);
        llvm::FunctionType *cloneFT = llvm::FunctionType::get(bitmapPtrTy, {bitmapPtrTy}, false);
        auto cloneFn = Module.getOrInsertFunction("roaring_bitmap_clone_runtime", cloneFT);
        if (bitmap->getType() != bitmapPtrTy)
            bitmap = Builder.CreateBitCast(bitmap, bitmapPtrTy);
        return Builder.CreateCall(cloneFn, {bitmap}, "set.clone");
    };
...
void IRGenVisitor::visitSetDecl(SetDeclNode *setDecl)
{
    ...
    if (setDecl->initializer->type != ASTNodeType::SetLiteral)
    {
        ...
        llvm::FunctionType *cloneFT = llvm::FunctionType::get(i8PtrTy, {i8PtrTy}, false);
        auto cloneFn = Module.getOrInsertFunction("roaring_bitmap_clone_runtime", cloneFT);
        bitmapPtr = Builder.CreateCall(cloneFn, {bitmapPtr}, baseName + ".clone");
