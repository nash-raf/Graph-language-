...
static inline void invalidate_select_cache(RoaringBitmap *bm)
{
    bm->select_cache_epoch = 0;
}

static inline void note_bitmap_mutation(RoaringBitmap *bm)
{
    bm->mutation_epoch++;
    if (bm->mutation_epoch == 0)
    {
        // Handle wrap-around by resetting to a valid non-zero epoch.
        bm->mutation_epoch = 1;
    }
    invalidate_select_cache(bm);
}

static bool ensure_select_cache_capacity(RoaringBitmap *bm, size_t required)
{
    if (required <= bm->select_cache_capacity)
    {
        return true;
    }

    size_t new_capacity = (bm->select_cache_capacity == 0) ? 1024 : bm->select_cache_capacity;
    ...
    uint32_t *new_cache = reinterpret_cast<uint32_t *>(
        std::realloc(bm->select_cache, new_capacity * sizeof(uint32_t)));
    if (!new_cache)
    {
        return false;
    }

    bm->select_cache = new_cache;
    bm->select_cache_capacity = new_capacity;
    return true;
}

static bool rebuild_select_cache(RoaringBitmap *bm)
{
    size_t total = 0;
    for (size_t i = 0; i < bm->num_containers; ++i)
    {
        const Container &c = bm->containers[i];
        ...
        {
            total += c.bitmap.cardinality;
        }
    }

    if (!ensure_select_cache_capacity(bm, total))
    {
        return false;
    }
    ...
    for (size_t i = 0; i < bm->num_containers; ++i)
    {
        ...
        if (c.type == ARRAY_CONTAINER)
        {
            ...
            {
                bm->select_cache[out++] = base + c.array.values[j];
            }
        }
        else if (c.type == BITMAP_CONTAINER)
        {
            ...
            for (uint32_t w = 0; w < 1024; ++w)
            {
                ...
                while (word)
                {
                    ...
                    bm->select_cache[out++] = base + (w * 64u + static_cast<uint32_t>(bit));
                    word &= word - 1;
                }
            }
        }
    ...
    bm->select_cache_size = out;
    bm->select_cache_epoch = bm->mutation_epoch;
    return true;
}
...
RoaringBitmap *roaring_bitmap_create(size_t arena_size, size_t initial_capacity)
{
    ...
    bm->mutation_epoch = 1;
    bm->select_cache_epoch = 0;
    bm->select_cache = nullptr;
    bm->select_cache_size = 0;
    bm->select_cache_capacity = 0;
...
Container *create_container(RoaringBitmap *bm, uint16_t key, ContainerType type)
{
    ...
    {
        size_t new_capacity = (bm->max_containers == 0) ? 8 : bm->max_containers * 2;
    ...
    if (type == BITMAP_CONTAINER)
    {
        ...
        container->bitmap.cardinality = 0;
    }
...
void promote_to_bitmap(RoaringBitmap *bm, Container *c)
{
    ...
    c->bitmap.bits = bits;
    c->bitmap.cardinality = old_cardinality;

    // fprintf(stderr, "[PROMOTE]   Success! bits=%p\n", (void *)c->bitmap.bits);
}
inline bool set_bit(BitmapContainer &b, uint16_t value)
{
    uint32_t byte_index = value >> 3;
    uint8_t bit_mask = 1u << (value & 7);
    bool inserted = (b.bits[byte_index] & bit_mask) == 0;
...
void roaring_bitmap_add(RoaringBitmap *bm, uint32_t value)
{
    ...
    {
        size_t before = container->array.cardinality;
        add_to_array(bm, container, low);
        mutated = (container->array.cardinality != before);
...
extern "C" RoaringBitmap *roaring_bitmap_intersect(RoaringBitmap *bm1, RoaringBitmap *bm2)
{
    ...
    while (i1 < bm1->num_containers && i2 < bm2->num_containers)
    {
        ...
        else
        {
            ...
            resultC->bitmap.cardinality = cardinality;
...
#pragma omp parallel for schedule(static)
            ...
            A->bitmap.cardinality = compute_bitmap_cardinality(A->bitmap.bits);
        }
...
extern "C" uint8_t *roaring_from_serialized(const uint8_t *data, uint64_t size)
{
    ...
    bm->max_containers = (num_containers < 8) ? 8 : num_containers; // min capacity 8
    bm->mutation_epoch = 1;
    bm->select_cache_epoch = 0;
    bm->select_cache = nullptr;
    bm->select_cache_size = 0;
    ...
    // Allocate container array (use max_containers for capacity)
    bm->containers = reinterpret_cast<Container *>(bm->arena.alloc(sizeof(Container) * bm->max_containers));
    if (!bm->containers)
    {
    ...
    std::memset(bm->containers, 0, sizeof(Container) * bm->max_containers);
...
extern "C" void roaring_print_edges(uint8_t *ptr, const int32_t *pairs, uint64_t pair_count)
{
    if (!ptr || !pairs || pair_count == 0)
    {
        printf("{}\n");
        return;
    }
    ...
    {
        uint32_t id = roaring_bitmap_get_at_index(bm, static_cast<uint32_t>(i));
        if (id >= pair_count)
            continue;

        int32_t u = pairs[id * 2];
        int32_t v = pairs[id * 2 + 1];
...
extern "C" void roaring_bitmap_remove(RoaringBitmap *bm, uint32_t value)
{
    ...
    else if (container->type == BITMAP_CONTAINER)
    {
        uint32_t byte_index = low >> 3;
        uint8_t bit_mask = 1u << (low & 7);
        if ((container->bitmap.bits[byte_index] & bit_mask) != 0)
        {
            container->bitmap.bits[byte_index] &= ~bit_mask;
            if (container->bitmap.cardinality > 0)
            {
                container->bitmap.cardinality--;
            }
            note_bitmap_mutation(bm);
...
extern "C" uint32_t roaring_bitmap_get_at_index(RoaringBitmap *bm, uint32_t index)
{
    ...
    if (bm->select_cache_epoch != bm->mutation_epoch)
    {
        if (!rebuild_select_cache(bm))
        {
            fprintf(stderr, "Error: Failed to build select cache in roaring_bitmap_get_at_index\n");
            return 0;
        }
    }

    if (index >= bm->select_cache_size)
    {
        fprintf(stderr, "Error: Index %u out of bounds (cardinality: %u)\n",
                index, static_cast<uint32_t>(bm->select_cache_size));
        return 0;
    }
