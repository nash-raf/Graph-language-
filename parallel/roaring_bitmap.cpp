...
struct ContainerMinHeap {
  ...
  const ContainerHeapNode &top() const { return nodes[0]; }
...
ContainerMinHeap *build_container_heap(RoaringBitmap **bitmaps, size_t count) {
  ...
  for (size_t i = 0; i < count; ++i) {
    if (bitmaps[i] && bitmaps[i]->num_containers > 0) {
      ContainerHeapNode node(&bitmaps[i]->containers[0], // First container
                             i,                          // Bitmap index
...
void advance_and_reinsert(ContainerMinHeap *heap, RoaringBitmap **bitmaps,
  ...
  if (next_idx < bitmaps[node.bitmap_idx]->num_containers) {
    ContainerHeapNode next_node(&bitmaps[node.bitmap_idx]->containers[next_idx],
                                node.bitmap_idx, next_idx);
    heap->push(next_node);
  }
...
RoaringBitmap *roaring_bitmap_create(size_t arena_size,
  ...
  // fprintf(stderr, "\n[CREATE] Creating RoaringBitmap with arena_size=%zu
  ...
  bm->containers = reinterpret_cast<Container *>(
      bm->arena.alloc(sizeof(Container) * initial_capacity));
  std::memset(bm->containers, 0, sizeof(Container) * initial_capacity);

  // fprintf(stderr, "[CREATE] Initial allocation: %zu bytes for container
...
//     // fprintf(stderr, "  Total: %d blocks, %zu/%zu bytes used (%.1f%%
//     full)\n",
//     //         block_count,
//     //         total_used,
//     //         total_capacity,
...
Container *create_container(RoaringBitmap *bm, uint16_t key,
                            ContainerType type) {
  // Resize if needed
  if (bm->num_containers >= bm->max_containers) {
    size_t new_capacity = bm->max_containers * 2;
    Container *new_array = reinterpret_cast<Container *>(
        bm->arena.alloc(sizeof(Container) * new_capacity));
    if (!new_array)
      return nullptr; // out of memory
    std::memcpy(new_array, bm->containers,
                sizeof(Container) * bm->num_containers);
  ...
  if (insert_pos < bm->num_containers) {
    std::memmove(&bm->containers[insert_pos + 1], &bm->containers[insert_pos],
...
void roaring_bitmap_free(RoaringBitmap *bm) {
  ...
  // bm->num_containers); print_arena_stats(bm->arena, "Before Free");
...
void roaring_bitmap_clear(RoaringBitmap *bm) {
  if (!bm)
    return;
  /* Do NOT call arena.free_all(): bm->containers is allocated from the arena.
   * Freeing would make containers a dangling pointer; the next add would crash.
   * Just reset the count so we reuse the containers array. */
  bm->num_containers = 0;
}
...
void add_to_array(RoaringBitmap *bm, Container *c, uint16_t value) {
  ...
  if (a.cardinality >= a.capacity) {
    ...
    uint16_t *new_values = reinterpret_cast<uint16_t *>(
        bm->arena.alloc(sizeof(uint16_t) * new_capacity));
    if (!new_values)
      return; // out of memory
  ...
  if (pos < a.cardinality) {
    std::memmove(a.values + pos + 1, a.values + pos,
                 sizeof(uint16_t) * (a.cardinality - pos));
  }
  a.values[pos] = value;
  a.cardinality++;
}
...
void promote_to_bitmap(RoaringBitmap *bm, Container *c) {
  ...
  // old_cardinality); fprintf(stderr, "[PROMOTE]   Arena state: %d blocks,
  // %zu/%zu bytes (%.1f%% full)\n",
  //         block_count, total_used, total_capacity, 100.0 * total_used /
  //         total_capacity);
...
void roaring_bitmap_add(RoaringBitmap *bm, uint32_t value) {
  ...
  if (container->type == ARRAY_CONTAINER) {
    ...
    if (container->array.cardinality > THRESHOLD) {
      // fprintf(stderr, "Promoting container with key=%u, cardinality=%zu to
      ...
      if (container->type == oldType) {
        ...
        // fprintf(stderr, "ERROR: Promotion failed! Still ARRAY_CONTAINER.
        // Stopping.\n");
        exit(1);
      }
      if (!container->bitmap.bits) {
...
inline PartitionResult partition_three_way_ptrs(Container **containers,
...
void quicksort_containers_desc_ptrs(Container **containers, size_t low,
                                    size_t high) {
  while (low < high) {
    if (high - low < 16) {
      for (size_t i = low + 1; i <= high; ++i) {
        ...
        while (j >= (int)low &&
               get_container_cardinality(containers[j]) < card_i) {
...
// Public interface: Sort array of container pointers in descending order by
// cardinality
...
extern "C" RoaringBitmap *roaring_bitmap_intersect(RoaringBitmap *bm1,
                                                   RoaringBitmap *bm2) {
  if (!bm1 || !bm2)
    return nullptr;
  ...
  RoaringBitmap *result =
      roaring_bitmap_create(128 * 1024, estimated_containers);
  ...
  while (i1 < bm1->num_containers && i2 < bm2->num_containers) {
    ...
    if (key1 < key2) {
      ...
      for (size_t word_idx = 0; word_idx < 256; ++word_idx) {
        __m256i vecA =
            _mm256_loadu_si256((__m256i const *)(bits1 + word_idx * 32));
        __m256i vecB =
            _mm256_loadu_si256((__m256i const *)(bits2 + word_idx * 32));
        __m256i vecC = _mm256_and_si256(vecA, vecB);
        _mm256_storeu_si256((__m256i *)(resultC->bitmap.bits + word_idx * 32),
                            vecC);
  ...
  if (total_cardinality <= 4096) {
    RoaringBitmap *finalResult =
        roaring_bitmap_create(8 * 1024, result->num_containers);

    for (size_t i = 0; i < result->num_containers; ++i) {
      Container *srcC = &result->containers[i];
      Container *destC =
          create_container(finalResult, srcC->key, ARRAY_CONTAINER);
      if (!destC) {
      ...
      for (uint16_t word_idx = 0; word_idx < 1024; ++word_idx) {
        ...
        while (w != 0) {
          ...
          add_to_array(finalResult, destC,
                       static_cast<uint16_t>(word_idx * 64 + r));
          w = w & (w - 1);
        }
      }
    }
...
extern "C" RoaringBitmap *roaring_bitmap_union(RoaringBitmap **bitmaps,
                                               size_t count) {
  if (!bitmaps || count == 0)
    return nullptr;
  ...
  while (!P->empty()) {
    ...
    if (T->num_containers >= T->max_containers) {
      ...
      Container *new_array = reinterpret_cast<Container *>(
          T->arena.alloc(sizeof(Container) * new_capacity));
      if (!new_array) {
        delete[] Q;
        return nullptr;
      }
      std::memcpy(new_array, T->containers,
                  sizeof(Container) * T->num_containers);
    ...
    if (insert_pos < T->num_containers) {
      std::memmove(&T->containers[insert_pos + 1], &T->containers[insert_pos],
    ...
    bool start_as_bitmap =
        (Q[0]->type == BITMAP_CONTAINER) ||
...
#pragma omp parallel for schedule(static)
      for (size_t word_idx = 0; word_idx < 256; ++word_idx) {
        __m256i result = _mm256_loadu_si256(
            (__m256i const *)(temp_bitmaps[0] + word_idx * 32));

        for (size_t i = 1; i < Q_size; ++i) {
          __m256i vec = _mm256_loadu_si256(
              (__m256i const *)(temp_bitmaps[i] + word_idx * 32));
          result = _mm256_or_si256(result, vec);
        }

        _mm256_storeu_si256((__m256i *)(A->bitmap.bits + word_idx * 32),
                            result);
      }
    ...
    } else // Array container path
    {
      ...
      A->array.values = reinterpret_cast<uint16_t *>(
          T->arena.alloc(sizeof(uint16_t) * A->array.capacity));
      ...
      std::memcpy(A->array.values, Q[0]->array.values,
                  sizeof(uint16_t) * A->array.cardinality);
      ...
      if (A->type == BITMAP_CONTAINER) {
        for (; i < Q_size; ++i) {
          if (Q[i]->type == BITMAP_CONTAINER) {
            for (size_t word_idx = 0; word_idx < 256; ++word_idx) {
              __m256i vecA = _mm256_loadu_si256(
                  (__m256i const *)(A->bitmap.bits + word_idx * 32));
              __m256i vecQ = _mm256_loadu_si256(
                  (__m256i const *)(Q[i]->bitmap.bits + word_idx * 32));
...
extern "C" void roaring_bitmap_portable_serialize(RoaringBitmap *bm,
                                                  uint8_t *buf) {
  if (!bm || !buf)
    return;
...
extern "C" uint8_t *roaring_from_serialized(const uint8_t *data,
                                            uint64_t size) {
  if (!data || size == 0)
    return nullptr;
  ...
  bm->containers = reinterpret_cast<Container *>(
      bm->arena.alloc(sizeof(Container) * num_containers));
  if (!bm->containers) {
    bm->arena.free_all();
    delete bm;
    return nullptr;
  ...
  for (size_t i = 0; i < num_containers; ++i) {
    ...
    // Read type
    c.type =
        static_cast<ContainerType>(*reinterpret_cast<const uint8_t *>(ptr));
    ptr += sizeof(uint8_t);
    ...
    if (c.type == BITMAP_CONTAINER) {
      ...
      c.array.values = reinterpret_cast<uint16_t *>(
          bm->arena.alloc(sizeof(uint16_t) * c.array.capacity));
      if (!c.array.values) {
        bm->arena.free_all();
        delete bm;
        return nullptr;
...
extern "C" void roaring_print_edges(uint8_t *ptr, const int32_t *pairs,
                                    uint64_t pair_count) {
  (void)ptr;
  (void)pairs;
  (void)pair_count;
  ...
  if (container->type == ARRAY_CONTAINER) {
    ...
    while (left < right) {
      ...
      if (container->array.values[mid] < low)
        ...
        std::memmove(
            container->array.values + mid, container->array.values + mid + 1,
...
extern "C" uint32_t roaring_bitmap_get_at_index(RoaringBitmap *bm,
                                                uint32_t index) {
  if (!bm) {
    return 0;
  }
