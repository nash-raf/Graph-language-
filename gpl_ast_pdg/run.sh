...
opt -load-pass-plugin=./CombinedPlugin.so \
    -load-pass-plugin=./LoopBoundExtractor.so \
    -passes="module(dep-graph-builder),function(loop-classify-print,loop-metadata-injector),function(mem2reg,loop-simplify,loop-rotate,lcssa,indvars,simplifycfg),function(loop-bound-extractor)" \
...
clang -O3 "${BASE}-optimized.ll" -o "${BASE}-exec"
