...
antlrcpp::Any ASTBuilder::visitStatement(BaseParser::StatementContext *ctx) {
  ...
  if (ctx->printStatement()) {
    ...
    auto *arrAlt = dynamic_cast<BaseParser::ArrayAssignStmtContext *>(
        ctx->arrayAssignStatement());
    if (!arrAlt) {
      throw std::runtime_error("internal: expected ArrayAssignStmtContext "
                               "(grammar alternative mismatch)");
    }
...
antlrcpp::Any ASTBuilder::visitBlock(BaseParser::BlockContext *ctx) {
  ...
  for (size_t i = 0; i < ctx->children.size(); ++i) {
    if (auto stmtCtx =
            dynamic_cast<BaseParser::StatementContext *>(ctx->children[i])) {
    ...
    } else if (auto retStmtCtx =
                   dynamic_cast<BaseParser::ReturnStatementContext *>(
                       ctx->children[i])) {
      ASTNodePtr exprNode =
          safe_any_cast<ASTNodePtr>(visitExpr(retStmtCtx->expr()));
...
antlrcpp::Any ASTBuilder::visitConditionalStatement(
    BaseParser::ConditionalStatementContext *ctx) {
  ASTNodePtr condition =
      safe_any_cast<ASTNodePtr>(visitCondition(ctx->condition()));
  ...
  auto condNode =
      std::make_shared<ConditionalNode>(condition, thenBlock, elseBlock);
...
antlrcpp::Any ASTBuilder::visitCondition(BaseParser::ConditionContext *ctx) {
  ...
  if (auto a = dynamic_cast<Ctx::LogicalAndContext *>(ctx)) {
    ASTNodePtr left =
        safe_any_cast<ASTNodePtr>(visitCondition(a->condition(0)));
    ASTNodePtr right =
        safe_any_cast<ASTNodePtr>(visitCondition(a->condition(1)));
    return ASTNodePtr(std::make_shared<BinaryExprNode>("&&", left, right));
  }
  if (auto o = dynamic_cast<Ctx::LogicalOrContext *>(ctx)) {
    ASTNodePtr left =
        safe_any_cast<ASTNodePtr>(visitCondition(o->condition(0)));
    ASTNodePtr right =
        safe_any_cast<ASTNodePtr>(visitCondition(o->condition(1)));
    return ASTNodePtr(std::make_shared<BinaryExprNode>("||", left, right));
  }
...
antlrcpp::Any
ASTBuilder::visitWhileStatement(BaseParser::WhileStatementContext *ctx) {
...
antlrcpp::Any ASTBuilder::visitExpr(BaseParser::ExprContext *ctx) {
  ...
  else if (auto addSubContext =
               dynamic_cast<BaseParser::AddSubExprContext *>(ctx)) {
  ...
  else if (auto intExprContext =
               dynamic_cast<BaseParser::IntExprContext *>(ctx)) {
    int val = std::stoi(intExprContext->getText());

    return ASTNodePtr(std::make_shared<IntLiteralNode>(val));
  } else if (auto idExprContext =
                 dynamic_cast<BaseParser::IdExprContext *>(ctx)) {
    std::string name = idExprContext->getText();

    return ASTNodePtr(std::make_shared<VariableNode>(name));
  } else if (auto parenContext =
                 dynamic_cast<BaseParser::ParenExprContext *>(ctx)) {

    return visitExpr(parenContext->expr());
  } else if (auto arrCtx =
                 dynamic_cast<BaseParser::ArrayAccessExprContext *>(ctx)) {
  ...
  } else if (auto containsCtx =
                 dynamic_cast<BaseParser::SetContainsExprContext *>(ctx)) {
    std::string setName = containsCtx->ID()->getText();
    ASTNodePtr arg = safe_any_cast<ASTNodePtr>(visitExpr(containsCtx->expr()));
    return ASTNodePtr(std::make_shared<SetContainsExprNode>(
        SetTargetKind::Variable, setName, arg));
  }
...
antlrcpp::Any ASTBuilder::visitVarDecl(BaseParser::VarDeclContext *ctx) {
  ...
  if (auto sd = dynamic_cast<BaseParser::SimpleDeclarationContext *>(ctx)) {
    ...
    if (init && init->type == ASTNodeType::ArrayLiteral) {
      ...
      return std::static_pointer_cast<ASTNode>(std::make_shared<VarDeclNode>(
          name, init, /*isArr=*/true, arrLit->elements.size()));
    } else {
      return std::static_pointer_cast<ASTNode>(
          std::make_shared<VarDeclNode>(name, init, /*isArr=*/false, 0));
    }
  } else if (auto arrayDecl =
                 dynamic_cast<BaseParser::ArrayDeclarationContext *>(ctx)) {
    ...
    int size = 0;
    if (auto sized =
            dynamic_cast<BaseParser::SizedArrayContext *>(declarator)) {
      name = sized->ID()->getText();
      size = std::stoi(sized->INT()->getText());
    } else if (auto unsized = dynamic_cast<BaseParser::UnsizedArrayContext *>(
                   declarator)) {
    ...
    return std::static_pointer_cast<ASTNode>(std::make_shared<VarDeclNode>(
        name, arrayLit, /*isArr=*/true, static_cast<size_t>(size)));
  }
  return nullptr;
}

antlrcpp::Any ASTBuilder::visitAssignmentStatement(
    BaseParser::AssignmentStatementContext *ctx) {
...
antlrcpp::Any ASTBuilder::visitFunction(BaseParser::FunctionContext *ctx) {
  ...
  auto funcNode =
      std::make_shared<FunctionDeclNode>(std::move(returnType), std::move(name),
...
antlrcpp::Any
ASTBuilder::visitArrayAssignStmt(BaseParser::ArrayAssignStmtContext *ctx) {
  ...
  ASTNodePtr assignNode =
      std::make_shared<AssignmentStmtNode>(arrAccess, valueNode);
  return std::static_pointer_cast<ASTNode>(assignNode);
}
...
antlrcpp::Any
ASTBuilder::visitFunctionCall(BaseParser::FunctionCallContext *ctx) {
...
antlrcpp::Any ASTBuilder::visitGraphDef(BaseParser::GraphDefContext *ctx) {
  ...
  auto gnode = std::make_shared<GraphDeclNode>(std::move(nm), std::move(nd),
                                               std::move(ed));
...
antlrcpp::Any
ASTBuilder::visitQueryStatement(BaseParser::QueryStatementContext *ctx) {
...
antlrcpp::Any
ASTBuilder::visitPrintStatement(BaseParser::PrintStatementContext *ctx) {
...
antlrcpp::Any ASTBuilder::visitSetDecl(BaseParser::SetDeclContext *ctx) {
  std::string name = ctx->ID()->getText();
  ...
  if (ctx->setInitializer()) {
    ...
    initNode = safe_any_cast<ASTNodePtr>(
        visitSetInitializer(ctx->setInitializer()), "visitSetDecl");
  }

  else if (ctx->setExpr()) {
    initNode =
        safe_any_cast<ASTNodePtr>(visitSetExpr(ctx->setExpr()), "visitSetDecl");
  }
...
antlrcpp::Any
ASTBuilder::visitSetInitializer(BaseParser::SetInitializerContext *ctx) {
  ...
  if (ctx->expr().size() > 0) {
    ...
    for (auto *e : ctx->expr()) {
      elements.push_back(
          safe_any_cast<ASTNodePtr>(visitExpr(e), "visitSetInitializer"));
    }
  }
...
antlrcpp::Any
ASTBuilder::visitSetOperation(BaseParser::SetOperationContext *ctx) {
  std::string targetName = ctx->ID()->getText();

  ASTNodePtr setExpr = safe_any_cast<ASTNodePtr>(visitSetExpr(ctx->setExpr()),
                                                 "visitSetOperation");
...
antlrcpp::Any ASTBuilder::visitSetExpr(BaseParser::SetExprContext *ctx) {
  ...
  } else if (auto intersectCtx =
                 dynamic_cast<BaseParser::SetIntersectContext *>(ctx)) {
    return visitSetIntersect(intersectCtx);
  } else if (auto nodesCtx =
                 dynamic_cast<BaseParser::GraphNodesSetContext *>(ctx)) {
    std::string gname = nodesCtx->graphID()->getText();
    return ASTNodePtr(
        std::make_shared<GraphMemberSetNode>(gname, GraphMemberKind::Nodes));
  } else if (auto edgesCtx =
                 dynamic_cast<BaseParser::GraphEdgesSetContext *>(ctx)) {
    std::string gname = edgesCtx->graphID()->getText();
    return ASTNodePtr(
        std::make_shared<GraphMemberSetNode>(gname, GraphMemberKind::Edges));
  } else if (auto idCtx = dynamic_cast<BaseParser::SetIdContext *>(ctx)) {
    return visitSetId(idCtx);
  } else if (auto literalCtx =
                 dynamic_cast<BaseParser::SetLiteralContext *>(ctx)) {
...
antlrcpp::Any ASTBuilder::visitSetUnion(BaseParser::SetUnionContext *ctx) {
  ASTNodePtr lhs = safe_any_cast<ASTNodePtr>(visitSetExpr(ctx->setExpr(0)),
                                             "visitSetUnion left");

  ASTNodePtr rhs = safe_any_cast<ASTNodePtr>(visitSetExpr(ctx->setExpr(1)),
                                             "visitSetUnion right");
...
antlrcpp::Any
ASTBuilder::visitSetIntersect(BaseParser::SetIntersectContext *ctx) {
  ASTNodePtr lhs = safe_any_cast<ASTNodePtr>(visitSetExpr(ctx->setExpr(0)),
...
antlrcpp::Any
ASTBuilder::visitSetMethodCall(BaseParser::SetMethodCallContext *ctx) {
  if (auto addCtx = dynamic_cast<BaseParser::SetAddMethodContext *>(ctx)) {
    return visitSetAddMethod(addCtx);
  } else if (auto removeCtx =
                 dynamic_cast<BaseParser::SetRemoveMethodContext *>(ctx)) {
...
antlrcpp::Any
ASTBuilder::visitSetAddMethod(BaseParser::SetAddMethodContext *ctx) {
  auto kind = parseSetTarget(ctx->setTarget());
  std::string name;
  if (ctx->setTarget()->ID())
    name = ctx->setTarget()->ID()->getText();
  else if (ctx->setTarget()->graphID())
    name = ctx->setTarget()->graphID()->getText();

  ASTNodePtr arg =
      safe_any_cast<ASTNodePtr>(visitExpr(ctx->expr()), "visitSetAddMethod");
...
antlrcpp::Any
ASTBuilder::visitSetRemoveMethod(BaseParser::SetRemoveMethodContext *ctx) {
  auto kind = parseSetTarget(ctx->setTarget());
  std::string name;
  if (ctx->setTarget()->ID())
    name = ctx->setTarget()->ID()->getText();
  ...
  ASTNodePtr arg =
      safe_any_cast<ASTNodePtr>(visitExpr(ctx->expr()), "visitSetRemoveMethod");
...
antlrcpp::Any
ASTBuilder::visitSetContainsExpr(BaseParser::SetContainsExprContext *ctx) {
  auto kind = parseSetTarget(ctx->setTarget());
  std::string name;
  if (ctx->setTarget()->ID())
    name = ctx->setTarget()->ID()->getText();
  ...
  ASTNodePtr arg =
      safe_any_cast<ASTNodePtr>(visitExpr(ctx->expr()), "visitSetContainsExpr");
