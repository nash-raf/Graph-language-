...
int injectConversions(Module &M, const std::vector<Region> &regions, const LayoutSchedule &S,
  ...
  for (const auto &KV : metaByGraphPtr)
    effectiveLayoutByGraph[KV.first] = LAYOUT_SET; /* graph starts in SET from autograph_init */
...
PreservedAnalyses AutoTunerModulePass::run(Module &M, ModuleAnalysisManager &MAM) {
  ...
  for (auto &KV : eventsByGraph) {
    ...
    /* Always call injectConversions when we have regions: we need at least the
     * initial SET->CSR conversion before the first traversal (graph starts in
     * LAYOUT_SET from autograph_init). */
