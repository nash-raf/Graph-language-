...
void IRGenVisitor::buildGlobalEdgeTable(ProgramNodePtr prog) {
  for (auto &node : prog->topLevel) {
    ...
    for (auto &e : G->edge_list) {
      getOrAddEdgeId(static_cast<int32_t>(e.first),
...
void IRGenVisitor::emitEdgePairsGlobal() {
  ...
  llvm::ArrayType *arrTy =
      llvm::ArrayType::get(i32Ty, GlobalEdgePairs.size() * 2);
  ...
  EdgePairsGV = new llvm::GlobalVariable(Module, arrTy, true,
                                         llvm::GlobalValue::PrivateLinkage,
                                         arrConst, "__edge_pairs");
...
IRGenVisitor::SetValueKind IRGenVisitor::inferSetKind(ASTNode *expr) {
  ...
  case ASTNodeType::GraphMemberSet: {
    ...
    return (gm->member == GraphMemberKind::Nodes) ? SetValueKind::Nodes
                                                  : SetValueKind::Edges;
  }
...
void IRGenVisitor::visitProgram(ProgramNodePtr prog) {
  ...
  for (auto &node : prog->topLevel) {
    if (node->type == ASTNodeType::FunctionDecl) {
      ...
      llvm::Function *fn = llvm::Function::Create(
          funcType, llvm::Function::ExternalLinkage, FD->name, &Module);
  ...
  auto *mainF = llvm::Function::Create(mainFT, llvm::Function::ExternalLinkage,
                                       "main", &Module);
  ...
  auto *BitmapPtrPtrTy = llvm::PointerType::get(BitmapPtrTy, 0);
  ...
  auto unionFT =
      llvm::FunctionType::get(BitmapPtrTy, {BitmapPtrPtrTy, i64Ty}, false);
  Module.getOrInsertFunction("roaring_bitmap_union", unionFT);

  // Intersect: takes two bitmaps
  auto intersectFT =
      llvm::FunctionType::get(BitmapPtrTy, {BitmapPtrTy, BitmapPtrTy}, false);
  ...
  auto getAtIndexFT =
      llvm::FunctionType::get(i32Ty, {BitmapPtrTy, i32Ty}, false);
  ...
  Module.getOrInsertFunction("roaring_bitmap_get_cardinality",
                             getCardinalityFT);
  ...
  for (auto &node : prog->topLevel) {
    ...
    case ASTNodeType::SetMethodCall:
      ...
      break;
    case ASTNodeType::GraphMutation:
      visitGraphMutation(static_cast<GraphMutationNode *>(node.get()));
      break;
    default:
...
void IRGenVisitor::visitFunctionDecl(FunctionDeclNode *funcDecl) {
  ...
  if (!Builder.GetInsertBlock()->getTerminator()) {
    if (funcDecl->returnType == "void") {
      ...
      Builder.CreateRet(
          llvm::ConstantInt::get(Builder.getInt32Ty(), // <-- no Context here
                                 0));
    }
  }
...
void IRGenVisitor::visitConditional(ConditionalNode *ifs) {
  ...
  // gets the current function. LLVM needs to know which function the blocks are
  // part of
  llvm::Function *parent = Builder.GetInsertBlock()->getParent();
  auto *thenBB = llvm::BasicBlock::Create(Context, "then", parent);
  auto *elseBB =
      ifs->elseBlock ? llvm::BasicBlock::Create(Context, "else") : nullptr;
...
void IRGenVisitor::visitStatement(ASTNode *node) {
  ...
  case ASTNodeType::SetMethodCall:
    ...
    break;
  case ASTNodeType::GraphMutation:
    visitGraphMutation(static_cast<GraphMutationNode *>(node));
    break;
  default:
...
void IRGenVisitor::visitGraphMutation(GraphMutationNode *node) {
  auto graphIt = GraphMap.find(node->graphName);
  if (graphIt == GraphMap.end()) {
    throw std::runtime_error("Undefined graph in mutation: " + node->graphName);
  }

  llvm::Value *graphPtr = graphIt->second;
  auto *ptrTy = llvm::PointerType::get(Context, 0);
  auto *i32Ty = Builder.getInt32Ty();
  auto *voidTy = Builder.getVoidTy();

  llvm::Value *nodesBmp = nullptr;
  llvm::Value *edgesBmp = nullptr;
  if (GraphNodesMap.count(node->graphName))
    nodesBmp = GraphNodesMap[node->graphName];
  if (GraphEdgesMap.count(node->graphName))
    edgesBmp = GraphEdgesMap[node->graphName];
  if (!nodesBmp)
    nodesBmp = llvm::ConstantPointerNull::get(llvm::cast<llvm::PointerType>(ptrTy));
  if (!edgesBmp)
    edgesBmp = llvm::ConstantPointerNull::get(llvm::cast<llvm::PointerType>(ptrTy));

  llvm::Value *edgePairsPtr = llvm::ConstantPointerNull::get(
      llvm::cast<llvm::PointerType>(ptrTy));
  if (EdgePairsGV && EdgePairsCount > 0) {
    auto *arrTy = llvm::cast<llvm::ArrayType>(EdgePairsGV->getValueType());
    llvm::Value *zero = Builder.getInt32(0);
    edgePairsPtr =
        Builder.CreateInBoundsGEP(arrTy, EdgePairsGV, {zero, zero}, "edge_pairs.ptr");
  }

  if (node->kind == GraphMutationKind::AddNode ||
      node->kind == GraphMutationKind::RemoveNode) {
    llvm::FunctionType *nodeFT =
        llvm::FunctionType::get(voidTy, {ptrTy, ptrTy, ptrTy, i32Ty}, false);
    auto addNodeFn = Module.getOrInsertFunction("graph_add_node", nodeFT);
    auto rmNodeFn = Module.getOrInsertFunction("graph_remove_node", nodeFT);

    for (int id : node->nodes) {
      llvm::Value *idV = llvm::ConstantInt::get(i32Ty, id);
      if (node->kind == GraphMutationKind::AddNode) {
        Builder.CreateCall(addNodeFn, {graphPtr, nodesBmp, edgePairsPtr, idV});
      } else {
        Builder.CreateCall(rmNodeFn, {graphPtr, nodesBmp, edgePairsPtr, idV});
      }
    }
    return;
  }
  ...
  for (auto const &e : node->edges) {
    ...
    if (node->kind == GraphMutationKind::AddEdge) {
...
void IRGenVisitor::visitAssignment(AssignmentStmtNode *assign) {
  // std::cerr << "[IRGen] visitAssignment called for LHS type = " <<
  // static_cast<int>(assign->lhs->type) << "\n";
  ...
  if (assign->lhs->type == ASTNodeType::Variable) {
    ...
    if (it == NamedValues.end())
      throw std::runtime_error(
          "IRGenVisitor: undefined variable in assignment: " + varNode->name);
    ...
    if (rhsVal->getType() != allocTy) {
      if (rhsVal->getType()->isIntegerTy() && allocTy->isIntegerTy())
        rhsVal = Builder.CreateIntCast(
            rhsVal, llvm::cast<llvm::IntegerType>(allocTy), true);
      else
        throw std::runtime_error(
            "IRGenVisitor: type mismatch in scalar assignment");
    }
  ...
  // Case 2: LHS is an array element access
    ...
    if (!baseVar)
      throw std::runtime_error(
          "IRGenVisitor: unsupported array base expression");

    auto it = NamedValues.find(baseVar->name);
    if (it == NamedValues.end())
      throw std::runtime_error("IRGenVisitor: undefined array in assignment: " +
                               baseVar->name);
    ...
    if (auto *arrTy =
            llvm::dyn_cast<llvm::ArrayType>(baseAlloca->getAllocatedType())) {
      llvm::Value *elemPtr = Builder.CreateGEP(arrTy,      // pointee type
                                               baseAlloca, // pointer
      ...
      if (rhsVal->getType() != elemTy) {
        if (rhsVal->getType()->isIntegerTy() && elemTy->isIntegerTy())
          rhsVal = Builder.CreateIntCast(
              rhsVal, llvm::cast<llvm::IntegerType>(elemTy), true);
        else
          throw std::runtime_error(
              "IRGenVisitor: type mismatch in array assignment");
      }
    ...
    // If it's a pointer to dynamically allocated array
    else if (auto *ptrTy = llvm::dyn_cast<llvm::PointerType>(
                 baseAlloca->getAllocatedType())) {
      llvm::Type *elemTy = ptrTy->getContainedType(0);

      llvm::Value *elemPtr = Builder.CreateGEP(elemTy,     // pointee type
                                               baseAlloca, // pointer
                                               {idxVal},   // index
                                               baseVar->name + "_elemptr");
      ...
      if (rhsVal->getType() != elemTy) {
        if (rhsVal->getType()->isIntegerTy() && elemTy->isIntegerTy())
          ...
          throw std::runtime_error(
              "IRGenVisitor: type mismatch in pointer array assignment");
      }
      ...
      throw std::runtime_error(
          "IRGenVisitor: unsupported array storage type for " + baseVar->name);
    }
  }
...
llvm::AllocaInst *IRGenVisitor::visitVarDecl(VarDeclNode *decl) {
  ...
  if (decl->isArray) {
    ...
    if (decl->initializer &&
      ...
      // If explicit size is 0 (shouldn't happen with our builder), infer from
      // initializer
      if (N == 0)
        N = arrLit->elements.size();
      // If initializer is larger than declared size, you may want to error or
      // truncate
    }

    if (N == 0) {
      throw std::runtime_error(
          "IRGenVisitor::visitVarDecl: array size is zero for " + decl->name);
    }
    ...
    // If initializer exists, store each initializer element and zero-fill
    // remaining
    if (decl->initializer &&
        decl->initializer->type == ASTNodeType::ArrayLiteral) {
      auto *arrLit = static_cast<ArrayLiteralNode *>(decl->initializer.get());
      ...
      for (size_t i = 0; i < M && i < N; ++i) {
        ...
        if (val->getType() != Builder.getInt32Ty() &&
          ...
          val = Builder.CreateIntCast(val, Builder.getInt32Ty(),
                                      /*isSigned=*/true);
        }
  ...
  if (decl->initializer) {
    ...
    if (initVal->getType() != Builder.getInt32Ty() &&
...
llvm::Value *IRGenVisitor::visitExpr(ASTNode *expr) {
  ...
  case ASTNodeType::ArrayAccess: {
    ...
    auto it = NamedValues.find(baseVar->name);
    if (it == NamedValues.end())
      throw std::runtime_error("Undefined array/set in access: " +
                               baseVar->name);
    llvm::AllocaInst *baseAlloca = it->second;
    ...
    if (allocatedType->isPointerTy()) {
      ...
      if (allocatedType == BitmapPtrTy) {
        ...
        llvm::Value *bitmapPtr = Builder.CreateLoad(BitmapPtrTy, baseAlloca,
                                                    baseVar->name + ".load");
        ...
        llvm::FunctionType *getAtIndexFT = llvm::FunctionType::get(
            Builder.getInt32Ty(), {BitmapPtrTy, Builder.getInt32Ty()}, false);

        auto getAtIndexFn = Module.getOrInsertFunction(
            "roaring_bitmap_get_at_index", getAtIndexFT);
    ...
    if (arrTy) {
      llvm::Value *elemPtr =
          Builder.CreateGEP(arrTy, baseAlloca, {Builder.getInt32(0), idxVal},
                            baseVar->name + "_elemptr");
      llvm::Type *elemTy = arrTy->getElementType();
      ...
      if (auto *ptrTy = llvm::dyn_cast<llvm::PointerType>(baseTy)) {
        llvm::Type *elemTy = ptrTy->getContainedType(0);
        llvm::Value *elemPtr = Builder.CreateGEP(elemTy, baseAlloca, {idxVal},
                                                 baseVar->name + "_elemptr");
        return Builder.CreateLoad(elemTy, elemPtr, baseVar->name + "_loadelem");
      }

      throw std::runtime_error(
          "IRGenVisitor: unsupported array storage type for load " +
          baseVar->name);
    }
  }
  default:
...
llvm::Value *IRGenVisitor::visitGraphDecl(GraphDeclNode *G) {
  ...
  auto *GV_RP = new llvm::GlobalVariable(Module, arrRP, true,
                                         llvm::GlobalValue::InternalLinkage,
                                         RPArray, G->name + "_csr_row");

  auto *GV_CI = new llvm::GlobalVariable(Module, arrCI, true,
                                         llvm::GlobalValue::InternalLinkage,
  ...
  llvm::FunctionCallee mallocCalle =
      Module.getOrInsertFunction("malloc", mallocTy);
  llvm::Function *mallocFn =
      llvm::cast<llvm::Function>(mallocCalle.getCallee());
  ...
  llvm::Value *rp_i8 = Builder.CreateBitCast(
      rowPtr, llvm::PointerType::getUnqual(llvm::Type::getInt8Ty(Context)));
  llvm::Value *ci_i8 = Builder.CreateBitCast(
      colPtr, llvm::PointerType::getUnqual(llvm::Type::getInt8Ty(Context)));
  llvm::Value *src_rp = Builder.CreateBitCast(
      GV_RP, llvm::PointerType::getUnqual(llvm::Type::getInt8Ty(Context)));
  llvm::Value *src_ci = Builder.CreateBitCast(
      GV_CI, llvm::PointerType::getUnqual(llvm::Type::getInt8Ty(Context)));
  ...
  Builder.CreateCall(
      memcpyFn,
      {rp_i8, src_rp, llvm::ConstantInt::get(I64, rowBytes), volatileFlag}, "");
  Builder.CreateCall(
      memcpyFn,
      {ci_i8, src_ci, llvm::ConstantInt::get(I64, colBytes), volatileFlag}, "");

  // Build the malloc prototype for allocating the Graph struct
  llvm::FunctionType *mallocFT_graph = llvm::FunctionType::get(
      llvm::PointerType::getUnqual(
          llvm::Type::getInt8Ty(Context)), // returns i8*
  ...
  llvm::Value *rawGraph =
      Builder.CreateCall(mallocFnG, graphSizeC, "graph_raw");

  auto *graphPtrTy = llvm::PointerType::get(GraphTy, 0);
  // Cast i8* → %struct.Graph*
  llvm::Value *graphPtr =
      Builder.CreateBitCast(rawGraph, graphPtrTy, "graph_ptr");
  ...
  // GEP + store into field 0: n
  {
    ...
    Builder.CreateStore(llvm::ConstantInt::get(Builder.getInt64Ty(), G->n),
                        nPtr);
  }
  ...
  // GEP + store into field 1: m
  {
    ...
    Builder.CreateStore(llvm::ConstantInt::get(Builder.getInt64Ty(), G->m),
                        mPtr);
  }
  ...
  auto emitBitmapFromBlob = [&](const std::vector<uint8_t> &blob,
    ...
    auto *blobGV = new llvm::GlobalVariable(Module, blobTy, true,
                                            llvm::GlobalValue::PrivateLinkage,
                                            blobConst, name);

    llvm::Value *zero = Builder.getInt32(0);
    llvm::Value *blobPtr =
        Builder.CreateInBoundsGEP(blobTy, blobGV, {zero, zero}, name + ".ptr");
    llvm::Value *sizeVal = llvm::ConstantInt::get(i64Ty, blob.size());
    return Builder.CreateCall(deserializeFn, {blobPtr, sizeVal},
                              name + ".bitmap");
  };

  auto edgesBlob = buildEdgeBlobForGraph(G);
  llvm::Value *nodesBmp =
      emitBitmapFromBlob(G->nodes_blob, G->name + "_nodes_blob");
  llvm::Value *edgesBmp =
      emitBitmapFromBlob(edgesBlob, G->name + "_edges_blob");

  GraphNodesMap[G->name] = nodesBmp;
  GraphEdgesMap[G->name] = edgesBmp;

  // -- NEW: Autotuner Initialization --
  // We must register the nodes_bitmap, edges_bitmap, and edge_pairs_table
  // with the autotuner so it can use LAYOUT_SET as the absolute base.

  // 1. Emit the edge_pairs_table (Array of {i64 u, i64 v}) with 0-based indices
  //    so autotuner build_csr_from_set produces CSR compatible with BFS (0-based)
  auto *i64TyInit = Builder.getInt64Ty();
  llvm::StructType *edgePairTy =
      llvm::StructType::get(Context, {i64TyInit, i64TyInit});
  llvm::ArrayType *pairsArrTy =
      llvm::ArrayType::get(edgePairTy, G->edge_list.size());

  std::map<int, int> id2idx;
  for (size_t i = 0; i < G->node_ids.size(); ++i)
    id2idx[G->node_ids[i]] = static_cast<int>(i);

  std::vector<llvm::Constant *> pairConsts;
  for (const auto &e : G->edge_list) {
    int u0 = id2idx.count(e.first) ? id2idx[e.first] : 0;
    int v0 = id2idx.count(e.second) ? id2idx[e.second] : 0;
    llvm::Constant *uC = llvm::ConstantInt::get(i64TyInit, u0);
    llvm::Constant *vC = llvm::ConstantInt::get(i64TyInit, v0);
    pairConsts.push_back(llvm::ConstantStruct::get(edgePairTy, {uC, vC}));
  }
  llvm::Constant *pairsArrConst =
      llvm::ConstantArray::get(pairsArrTy, pairConsts);
  auto *pairsGV = new llvm::GlobalVariable(
      Module, pairsArrTy, true, llvm::GlobalValue::PrivateLinkage,
      pairsArrConst, G->name + "_edge_pairs");

  llvm::Value *zero = Builder.getInt32(0);
  llvm::Value *pairsPtr = Builder.CreateInBoundsGEP(
      pairsArrTy, pairsGV, {zero, zero}, G->name + "_pairs_ptr");

  // 2. Call autograph_init(graphPtr, n, m, nodes_bmp, edges_bmp, pairsPtr)
  auto *voidTyInit = llvm::Type::getVoidTy(Context);
  auto *ptrTyInit = llvm::PointerType::get(Context, 0);

  llvm::FunctionType *initFT = llvm::FunctionType::get(
      voidTyInit,
      {ptrTyInit, i64TyInit, i64TyInit, ptrTyInit, ptrTyInit, ptrTyInit},
      false);
  auto initFn = Module.getOrInsertFunction("autograph_init", initFT);

  llvm::Value *nValInit = llvm::ConstantInt::get(i64TyInit, G->n);
  llvm::Value *mValInit = llvm::ConstantInt::get(i64TyInit, G->m);

  Builder.CreateCall(
      initFn, {graphPtr, nValInit, mValInit, nodesBmp, edgesBmp, pairsPtr});

  GraphEdgePairsMap[G->name] = pairsPtr;
  return graphPtr;
}
...
void IRGenVisitor::visitQuery(QueryNode *Q) {
  ...
  // Ensure layout is CSR before BFS (graph starts in SET from autograph_init;
  // autotuner may skip injection when all-CSR, so we must ensure layout here).
  auto *i64Ty = Builder.getInt64Ty();
  auto *i32Ty = Builder.getInt32Ty();
  auto *ptrTy = llvm::PointerType::get(Context, 0);
  llvm::Value *nVal = Builder.CreateLoad(
      i64Ty, Builder.CreateStructGEP(GraphTy, graphPtr, 0), "g.n");
  llvm::Value *mVal = Builder.CreateLoad(
      i64Ty, Builder.CreateStructGEP(GraphTy, graphPtr, 1), "g.m");
  llvm::Value *nullPtr = llvm::ConstantPointerNull::get(ptrTy);
  llvm::Value *layoutCSR = llvm::ConstantInt::get(i32Ty, 0);  // LAYOUT_CSR
  llvm::FunctionType *ensureFT = llvm::FunctionType::get(
      llvm::Type::getVoidTy(Context),
      {ptrTy, i64Ty, i64Ty, ptrTy, ptrTy, ptrTy, ptrTy, ptrTy, i32Ty},
      false);
  auto ensureFn = Module.getOrInsertFunction("autograph_ensure_layout", ensureFT);
  llvm::Value *nodesBmp = GraphNodesMap.at(Q->graphName);
  llvm::Value *edgesBmp = GraphEdgesMap.at(Q->graphName);
  llvm::Value *pairsPtr = GraphEdgePairsMap.at(Q->graphName);
  Builder.CreateCall(ensureFn,
      {graphPtr, nVal, mVal, nullPtr, nullPtr,
       nodesBmp, edgesBmp, pairsPtr, layoutCSR});

  // Declare and call bfs_runtime
  llvm::Type *voidTy = llvm::Type::getVoidTy(Context);
  llvm::Type *graphPtrTy = llvm::PointerType::get(GraphTy, 0);
  llvm::FunctionType *bfsFT =
      llvm::FunctionType::get(voidTy, {graphPtrTy}, /*isVarArg=*/false);
...
//         // int printf(char*, ...)
//         auto *i8p =
//         llvm::PointerType::getUnqual(llvm::Type::getInt8Ty(Context)); auto
...
//                 llvm::Value *ptr = Builder.CreateLoad(i8PtrTy, alloca,
//                 var->name + ".ptr"); llvm::FunctionType *printFT =
//                 llvm::FunctionType::get(Builder.getVoidTy(), {i8PtrTy},
//                 false); auto printFn =
//                 Module.getOrInsertFunction("roaring_print", printFT);
...
//         // use "%s\n" format.
//         throw std::runtime_error("print: only int literals & vars supported
//         so far");
//     }
...
void IRGenVisitor::visitPrintStmt(PrintStmtNode *node) {
  ...
  if (exprNode->type == ASTNodeType::Variable) {
    ...
    if (llvm::isa<llvm::PointerType>(allocatedTy)) {
      auto *BitmapPtrTy = getBitmapPtrTy(Context);
      llvm::Value *ptr =
          Builder.CreateLoad(BitmapPtrTy, alloca, var->name + ".bitmap");

      auto kindIt = SetKinds.find(var->name);
      auto kind =
          (kindIt == SetKinds.end()) ? SetValueKind::Unknown : kindIt->second;
      ...
      if (kind == SetValueKind::Edges && EdgePairsGV && EdgePairsCount > 0) {
        ...
        llvm::Value *zero = Builder.getInt32(0);
        llvm::Value *pairsPtr =
            Builder.CreateInBoundsGEP(EdgePairsGV->getValueType(), EdgePairsGV,
                                      {zero, zero}, "edge_pairs.ptr");

        llvm::Value *countVal = llvm::ConstantInt::get(i64Ty, EdgePairsCount);

        llvm::FunctionType *printEdgesFT = llvm::FunctionType::get(
            Builder.getVoidTy(), {BitmapPtrTy, i32PtrTy, i64Ty}, false);

        auto printEdgesFn =
            Module.getOrInsertFunction("roaring_print_edges", printEdgesFT);
        Builder.CreateCall(printEdgesFn, {ptr, pairsPtr, countVal});
        return;
      }

      llvm::FunctionType *printFT =
          llvm::FunctionType::get(Builder.getVoidTy(), {BitmapPtrTy}, false);
    ...
    llvm::Value *val =
        Builder.CreateLoad(Builder.getInt32Ty(), alloca, var->name);
    llvm::Function *printfFn = Module.getFunction("printf");
    if (!printfFn) {
      llvm::FunctionType *printfTy = llvm::FunctionType::get(
          Builder.getInt32Ty(), {llvm::PointerType::get(Context, 0)}, true);
      printfFn = llvm::Function::Create(
          printfTy, llvm::Function::ExternalLinkage, "printf", Module);
    }
  ...
  throw std::runtime_error(
      "print only supports variables (int or set) for now");
}
...
void IRGenVisitor::visitSetDecl(SetDeclNode *setDecl) {
  ...
  llvm::AllocaInst *bitmapAlloca =
      tmpB.CreateAlloca(i8PtrTy, nullptr, baseName);
  NamedValues[baseName] = bitmapAlloca;
  SetKinds[setDecl->name] = inferSetKind(setDecl->initializer.get());

  if (setDecl->initializer &&
      setDecl->initializer->type != ASTNodeType::SetLiteral) {
    llvm::Value *bitmapPtr = visitSetExpr(setDecl->initializer.get());
    Builder.CreateStore(bitmapPtr, bitmapAlloca);
    return;
  }

  // Check if initializer is a set expression (for cases like: set s3 = s1 union
  // s2;) if (setDecl->initializer && setDecl->initializer->type ==
  // ASTNodeType::SetBinaryExpr)
  // {
  ...
  llvm::GlobalVariable *blobGV = new llvm::GlobalVariable(
      Module, blobTy,
      /*isConstant=*/true, llvm::GlobalValue::PrivateLinkage, blobConst,
      blobName);

  llvm::Value *zero = Builder.getInt32(0);
  llvm::Value *blobPtr = Builder.CreateInBoundsGEP(blobTy, blobGV, {zero, zero},
                                                   blobName + ".ptr");
  ...
  llvm::Value *sizeVal = llvm::ConstantInt::get(i64Ty, blobSize);

  llvm::Value *bitmapPtr = Builder.CreateCall(deserializeFn, {blobPtr, sizeVal},
                                              baseName + ".bitmap");
...
llvm::Value *IRGenVisitor::visitSetExpr(ASTNode *expr) {
  ...
  case ASTNodeType::GraphMemberSet: {
    auto *gm = static_cast<GraphMemberSetNode *>(expr);
    llvm::Value *sourceBitmap = nullptr;
    if (gm->member == GraphMemberKind::Nodes)
      sourceBitmap = GraphNodesMap.at(gm->graphName);
    else
      sourceBitmap = GraphEdgesMap.at(gm->graphName);

    // Deep copy the bitmap instead of aliasing it
    auto *BitmapPtrTy = getBitmapPtrTy(Context);
    llvm::FunctionType *copyFT =
        llvm::FunctionType::get(BitmapPtrTy, {BitmapPtrTy}, false);
    auto copyFn = Module.getOrInsertFunction("roaring_bitmap_copy", copyFT);

    return Builder.CreateCall(copyFn, {sourceBitmap}, "set.copy");
  }
  ...
  case ASTNodeType::Variable: {
    ...
    auto *BitmapPtrTy = getBitmapPtrTy(Context);
    llvm::Value *sourceBitmap =
        Builder.CreateLoad(BitmapPtrTy, it->second, varNode->name + ".load");
  ...
  case ASTNodeType::SetLiteral: {
    ...
    llvm::FunctionType *createFT =
        llvm::FunctionType::get(BitmapPtrTy, {i64Ty, i64Ty}, false);
    ...
    llvm::Value *tempBitmap =
        Builder.CreateCall(createFn, {arenaSize, initialCap}, "temp.bitmap");

    llvm::FunctionType *addFT = llvm::FunctionType::get(
        llvm::Type::getVoidTy(Context), {BitmapPtrTy, i32Ty}, false);
...
llvm::SmallVector<ASTNode *, 8>
IRGenVisitor::flattenSetOperation(ASTNode *expr, const std::string &targetOp) {
...
llvm::Value *IRGenVisitor::visitSetBinaryExpr(SetBinaryExprNode *binExpr) {
  ...
  if (binExpr->op == "union") {
    llvm::SmallVector<ASTNode *, 8> operands =
        flattenSetOperation(binExpr, "union");
    ...
    llvm::Value *arraySize =
        llvm::ConstantInt::get(Builder.getInt64Ty(), operands.size());
    llvm::AllocaInst *bitmapArray =
        Builder.CreateAlloca(BitmapPtrTy, arraySize, "bitmap_array");

    for (size_t i = 0; i < bitmapPtrs.size(); ++i) {
      llvm::Value *idx = llvm::ConstantInt::get(Builder.getInt32Ty(), i);
      llvm::Value *elemPtr = Builder.CreateGEP(
          BitmapPtrTy, bitmapArray, {idx}, "array_elem_" + std::to_string(i));
      Builder.CreateStore(bitmapPtrs[i], elemPtr);
    }

    llvm::FunctionType *unionFT = llvm::FunctionType::get(
        BitmapPtrTy, {llvm::PointerType::get(BitmapPtrTy, 0), i64Ty}, false);
    ...
    llvm::Value *resultBitmap =
        Builder.CreateCall(unionFn, {bitmapArray, count}, "set.union.result");
    ...
    for (size_t i = 0; i < bitmapPtrs.size(); ++i) {
      if (operands[i]->type != ASTNodeType::Variable &&
          operands[i]->type != ASTNodeType::GraphMemberSet) {
    ...
    llvm::Value *resultBitmap = Builder.CreateCall(
        intersectFn, {lhsBitmap, rhsBitmap}, "set.intersect.result");
    llvm::FunctionType *freeFT =
        llvm::FunctionType::get(Builder.getVoidTy(), {BitmapPtrTy}, false);
    auto freeFn = Module.getOrInsertFunction("roaring_bitmap_free", freeFT);

    if (binExpr->lhs->type != ASTNodeType::Variable &&
        binExpr->lhs->type != ASTNodeType::GraphMemberSet) {
      Builder.CreateCall(freeFn, {lhsBitmap});
    }

    if (binExpr->rhs->type != ASTNodeType::Variable &&
        binExpr->rhs->type != ASTNodeType::GraphMemberSet) {
...
void IRGenVisitor::visitSetMethodCall(SetMethodCallNode *node) {
  ...
  if (node->targetKind == SetTargetKind::Variable) {
    ...
    bitmapPtr =
        Builder.CreateLoad(BitmapPtrTy, it->second, node->targetName + ".load");
  ...
  // llvm::Value *bitmapPtr = Builder.CreateLoad(BitmapPtrTy, setAlloca,
  // node->setName + ".load");
  ...
  if (!printfFn) {
    llvm::FunctionType *printfTy = llvm::FunctionType::get(
        Builder.getInt32Ty(), {llvm::PointerType::get(Context, 0)}, true);
    printfFn = llvm::Function::Create(printfTy, llvm::Function::ExternalLinkage,
                                      "printf", Module);
  }
  ...
  if (node->methodName == "add") {
    ...
    llvm::Value *debugStr = Builder.CreateGlobalStringPtr(
        "[DEBUG] Calling roaring_bitmap_add(%p, %d)\n");
    Builder.CreateCall(printfFn, {debugStr, bitmapPtr, argValue});

    // Call roaring_bitmap_add(bitmap*, i32)
    llvm::FunctionType *addFT =
        llvm::FunctionType::get(voidTy, {BitmapPtrTy, i32Ty}, false);
    ...
    llvm::Value *afterStr =
        Builder.CreateGlobalStringPtr("[DEBUG] After add, printing bitmap:\n");
    ...
    // Debug output
    llvm::Value *debugStr = Builder.CreateGlobalStringPtr(
        "[DEBUG] Calling roaring_bitmap_remove(%p, %d)\n");
    Builder.CreateCall(printfFn, {debugStr, bitmapPtr, argValue});

    // Call roaring_bitmap_remove(bitmap*, i32)
    llvm::FunctionType *removeFT =
        llvm::FunctionType::get(voidTy, {BitmapPtrTy, i32Ty}, false);
    ...
    llvm::Value *afterStr = Builder.CreateGlobalStringPtr(
        "[DEBUG] After remove, printing bitmap:\n");
    Builder.CreateCall(printfFn, {afterStr});
...
llvm::Value *IRGenVisitor::visitSetContainsExpr(SetContainsExprNode *node) {
  ...
  if (node->targetKind == SetTargetKind::Variable) {
    ...
    llvm::AllocaInst *setAlloca = it->second;
    bitmapPtr =
        Builder.CreateLoad(BitmapPtrTy, setAlloca, node->targetName + ".load");
  } else if (node->targetKind == SetTargetKind::GraphNodes) {
    bitmapPtr = GraphNodesMap.at(node->targetName);
  } else {
    bitmapPtr = GraphEdgesMap.at(node->targetName);
  ...
  llvm::FunctionType *containsFT =
      llvm::FunctionType::get(i32Ty, {BitmapPtrTy, i32Ty}, false);

  auto containsFn =
      Module.getOrInsertFunction("roaring_bitmap_contains", containsFT);
  llvm::Value *result =
      Builder.CreateCall(containsFn, {bitmapPtr, argValue}, "contains.result");

  return Builder.CreateICmpNE(result, llvm::ConstantInt::get(i32Ty, 0),
                              "contains.bool");
}