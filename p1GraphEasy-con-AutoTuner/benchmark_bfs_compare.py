...
def main():
    ...
    note = f"1M vertices, 1M undirected input edges, source 0; warm-up + median of {args.rounds} alternating launches"
    if args.mode == "e2e":
        ...
        rows = []
        for t in range(1, args.max_threads+1):
            s, g = paired(t, args.rounds)
            rows.append({"threads": t, "sgpl_median_ms": med(s),
                         "graptor_median_ms": med(g)})
            print(f"{t}: SGPL {med(s):.3f} ms; Graptor {med(g):.3f} ms")
