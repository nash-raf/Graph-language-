...
llvm::Function *outlineLoop(llvm::Function &F, llvm::LoopInfo &LI, llvm::DominatorTree &DT, llvm::ScalarEvolution &SE, llvm::Value *&StartV, llvm::Value *&EndV, llvm::Value *&StepV)
{
    ...
    for (llvm::Loop *L : Innermost)
    {
        ...
        if (L->getHeader()->getTerminator())
        {
            if (llvm::MDNode *MD = L->getHeader()->getTerminator()->getMetadata("my.loop.parallel"))
            {
                for (unsigned i = 0; i < llvm::LoopMD->getNumOperands(); ++i)
                {
                    if (auto ConstMD = llvm::dyn_cast<llvm::ConstantAsMetadata>(LoopMD->getOperand(i)))
                    {
                        if (autoCI = llvm::dyn_cast<llvm::ConstantInt>(ConstMD->getValue()))
                        {
                            bool Parallel = CI->getZExtValue() != 0;
                            errs() << "Loop parallel metadata = " << Parallel << "\n";
                        }
                    }
                    else if (auto *MDN = llvm::dyn_cast<llvm::MDNode>(LoopMD->getOperand(i)))
                    {
                        errs() << "Nested metadata node with " << MDN->getNumOperands() << " operands\n";
                    }
                }
            }
        }
