...
int main(int argc, char **argv)
{
    ...
    {
        llvm::SMDiagnostic Err;
        std::unique_ptr<llvm::Module> MutMod = llvm::parseIRFile("graph_mutation_runtime.ll", Err, Ctx);
        if (!MutMod)
        {
            Err.print("GraphProgram", llvm::errs());
    ...
    {
        llvm::SMDiagnostic Err;
        std::unique_ptr<llvm::Module> AutoMod = llvm::parseIRFile("autotuner_runtime.ll", Err, Ctx);
        if (!AutoMod)
        {
            Err.print("GraphProgram", llvm::errs());
        ...
        llvm::Linker L(*M);
        if (L.linkInModule(std::move(AutoMod)))
        {
