...
static std::string cache_path_for(const char *filename, bool weighted) {
    return std::string(filename) +
           (weighted ? ".weighted.sgplbin" : ".sgplbin");
}
...
static Graph *load_graph_from_file_impl(const char *filename, bool weighted) {
    ...
    std::string cache_path = cache_path_for(filename, weighted);
    bool cache_disabled = graph_cache_disabled();
    if (!cache_disabled && !graph_cache_refresh_requested()) {
        Graph *cached =
            try_load_graph_cache(cache_path, source_stat, weighted);
        if (cached)
            return cached;
    }
    ...
    Graph *graph =
        build_parallel_csr(edges, thread_offsets, n, weighted);
