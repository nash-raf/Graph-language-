...
/* Defined in graph_loader_runtime.cpp — materializes mmap-backed arrays. */
extern void graph_ensure_owned_storage(Graph *g);
...
void graph_add_edge(void *graph_ptr, void *edges_bmp, int32_t from, int32_t to,
  ...
  case LAYOUT_BCSR:
  {
    ...
    if (added_ab > 0 && added_ba > 0)
      autograph_mark_canonical_dirty(graph_ptr);
    if (!in_bounds) {
      /* OOB endpoints are not addressable in the current BCSR vertex space.
       * Park on SET, record the edge, then rebuild BCSR from canonical sets
       * (same pattern as CSR/PCSR OOB). */
      autograph_ensure_layout_set(graph_ptr);
      autograph_canonical_add_edge(graph_ptr, from, to);
      autograph_ensure_layout(graph_ptr, g->n, g->m, g->row_ptr, g->col_idx,
                              NULL, edges_bmp, NULL, LAYOUT_BCSR);
    }
    break;
  }
