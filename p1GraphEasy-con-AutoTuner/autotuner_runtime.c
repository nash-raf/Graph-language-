...
static void build_csr_from_meta(AutoGraphMeta *meta, int64_t *out_n, int64_t **out_rp,
  ...
  for (int64_t e = 0; e < meta->static_pair_count; e++) {
    ...
    if (u >= 0 && v >= 0 && (int64_t)u < dense_n && (int64_t)v < dense_n &&
        roaring_bitmap_contains(nb, (uint32_t)u) &&
        roaring_bitmap_contains(nb, (uint32_t)v)) {
      rp[(int64_t)u + 1]++;
      edge_count++;
    }
  }
...
void autograph_ensure_layout_set(void *graph_ptr) {
  ...
  {
    RoaringBitmap *nb2 = (RoaringBitmap *)meta->nodes_bitmap;
    uint64_t card = roaring_bitmap_get_cardinality(nb2);
    if (card == 0) {
      nn = 0;
    } else {
      nn = (int64_t)roaring_bitmap_get_at_index(nb2, card - 1) + 1;
    }
  }
  mm = 0;
  RoaringBitmap *eb = (RoaringBitmap *)meta->edges_bitmap;
