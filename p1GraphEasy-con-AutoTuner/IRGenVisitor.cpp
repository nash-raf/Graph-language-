...
// Base address of a 2D array's element storage. Top-level 2D arrays are kept as
// a global holding a pointer to the real allocation; inside a function the
// storage is the allocation itself.
...
/* ═══════════════════════════════════════════════════════════════════════════
 * Dense semiring-closure motif (ported from p2-AT).
 *
 * Recognizes a Floyd–Warshall / Kleene triple nest over a 2D matrix and routes
 * it to autograph_closure, parameterized by which closed semiring the body
 * implements.  Frontier EdgeMap (above) stays the sparse path; this is the
 * dense counterpart.
 *
 *     OP  REL   semiring
 *     +   <     (min, +)   APSP
 *     +   >     (max, +)   longest / critical path
 *     *   >     (max, ×)   transitive closure / max-reliability
 *     *   <     (min, ×)   min-product
 * ═══════════════════════════════════════════════════════════════════════════ */
...
void IRGenVisitor::visitAssignment(AssignmentStmtNode *assign)
{
    ...
    else if (assign->lhs->type == ASTNodeType::Array2DAccess)
    {
        ...
        // Delinearizable addressing: widen to i64 and emit the row step and the
        // column step as SEPARATE GEPs.  The previous form computed one flat
        // i32 index (row * cols + col) and applied it in a single GEP, which
        // defeats Polly twice over -- the 2D structure is gone, and an i32
        // subscript may wrap, so the access is not provably affine.  Measured
        // on a 512x512 matmul, stock `opt -polly` found 0 SCoPs with the flat
        // form and 218 polly.* references with this one.
        auto *i64TyAddr = Builder.getInt64Ty();
        llvm::Value *cols64 =
            Builder.CreateIntCast(metaIt->second.colsVal, i64TyAddr, true, "dim64");
        llvm::Value *row64 = Builder.CreateIntCast(rowIdx, i64TyAddr, true, "row64");
...
void IRGenVisitor::visitWhile(WhileStmtNode *ws)
{
    ...
    auto emitSemiringClosure = [&](const SemiringClosurePattern &pattern) {
        ...
        // Engine addresses D as flat n×n; rectangular or mismatched extent
        // falls through to ordinary lowering via a runtime guard.
...
void IRGenVisitor::visitForEach(ForEachStmtNode *fs)
{
    ...
    if (fs->targetType == ForEachTargetType::Neighbor)
    {
        ...
        // Allocate the user-visible neighbor variable.
        llvm::Function *parent = Builder.GetInsertBlock()->getParent();
        llvm::IRBuilder<> TmpB(&parent->getEntryBlock(), parent->getEntryBlock().begin());
        auto *userAlloca = TmpB.CreateAlloca(i32Ty, nullptr, fs->var1);
        NamedValues[fs->var1] = userAlloca;
        ...
        // Must match AutoNeighborIter in autotuner_runtime.h (layout+epoch after state).
        // Undersizing this alloca lets init write past the end; the loop body then
        // clobbers layout/epoch and iter_next stops after the first neighbor.
        auto *iterStructTy = llvm::StructType::get(
            Context,
            {opaquePtrTy,  // meta
             i64Ty,        // u
             i64Ty,        // pos
             ...
             i32Ty},       // pad
            false);
        auto *iterPtrTy = llvm::PointerType::get(iterStructTy, 0);

        // Allocate iterator on the stack
        auto *iterAlloca = TmpB.CreateAlloca(iterStructTy, nullptr, "nbr_iter");

        // Declare autograph_neighbor_iter_init(void*, i64, AutoNeighborIter*)
        llvm::FunctionType *initFT = llvm::FunctionType::get(
            voidTy, {opaquePtrTy, i64Ty, iterPtrTy}, false);
        auto initFn = Module.getOrInsertFunction("autograph_neighbor_iter_init", initFT);

        // Declare autograph_neighbor_iter_next(AutoNeighborIter*, i32*) -> i32
        llvm::FunctionType *nextFT = llvm::FunctionType::get(
            i32Ty, {iterPtrTy, i32PtrTyLocal}, false);
        auto nextFn = Module.getOrInsertFunction("autograph_neighbor_iter_next", nextFT);
        ...
        // Preheader: call autograph_neighbor_iter_init(G, v, &iter)
        llvm::BasicBlock *preheaderBB = Builder.GetInsertBlock();
        Builder.CreateCall(initFn, {graphPtr, srcVertex, iterAlloca});
        Builder.CreateBr(condBB);

        // Condition: autograph_neighbor_iter_next(&iter, &userVar) != 0
        Builder.SetInsertPoint(condBB);
        llvm::Value *hasNext = Builder.CreateCall(nextFn, {iterAlloca, userAlloca}, "nbr_has_next");
        llvm::Value *cmp = Builder.CreateICmpNE(hasNext, llvm::ConstantInt::get(i32Ty, 0), "nbr_cond");
        ...
        // Body: userVar is already filled by iter_next
        Builder.SetInsertPoint(bodyBB);
        visitBlock(static_cast<BlockStmtNode *>(fs->body.get()));
        if (!Builder.GetInsertBlock()->getTerminator())
            Builder.CreateBr(condBB);
    ...
    if (fs->targetType == ForEachTargetType::Edge)
    {
        ...
        // Must match AutoNeighborIter in autotuner_runtime.h (see neighbor-foreach above).
        auto *iterStructTy = llvm::StructType::get(
            Context,
            {opaquePtrTy,  // meta
             i64Ty,        // u
        ...
        // Declare autograph_neighbor_iter_init and next
        llvm::FunctionType *initFT = llvm::FunctionType::get(
            voidTy, {opaquePtrTy, i64Ty, iterPtrTy}, false);
        auto initFn = Module.getOrInsertFunction("autograph_neighbor_iter_init", initFT);
        llvm::FunctionType *nextFT = llvm::FunctionType::get(
        ...
        // Outer body: set u, init iterator, start inner loop
        ...
        // Allocate iterator on the stack (per vertex)
        auto *iterAlloca = Builder.CreateAlloca(iterStructTy, nullptr, "edge_iter");
        llvm::Value *uCur = Builder.CreateLoad(i64Ty, uIdxAlloca, "u_cur");
        Builder.CreateCall(initFn, {graphPtr, uCur, iterAlloca});
        ...
        // Inner condition: iter_next(&iter, &v) != 0
        Builder.SetInsertPoint(innerCondBB);
        llvm::Value *hasNext = Builder.CreateCall(nextFn, {iterAlloca, var2Alloca}, "edge_has_next");
        ...
        // User body: execute user loop body (v already in var2Alloca)
...
llvm::Value *IRGenVisitor::visitWeightedGraphDecl(WeightedGraphDeclNode *G)
{
    ...
    if (G->isFileGraph)
    {
        ...
        llvm::FunctionType *loadFT = llvm::FunctionType::get(
            graphPtrTy, {i8PtrTy}, false);
        auto loadFn = Module.getOrInsertFunction(
            G->directed ? "load_weighted_graph_from_file_directed"
                        : "load_weighted_graph_from_file",
            loadFT);
